...
/**
 * Static shading abstraction to easily use different lighting values.
 */
public record StaticShading(double x, double down, double up, double z) {
    public static final StaticShading WORLD = new StaticShading(0.6, 0.5, 1.0, 0.8);

    public static final StaticShading SLOT = new StaticShading(0.5, 0.5, 1.0, 0.6);
     ...
     */
    public double quadDirectionalShade(
        final double ax,
        final double ay,
        final double az,
        final double bx,
     ...
     */
    public double normalVectorShade(final double x, final double y, final double z) {
    ...
    public double unscaledNormalVectorShade(
        final double x,
        final double y,
        final double z
    ) {
        // Calculate shade per component.
        final double shadeX = this.x * Math.abs(x);
        final double shadeY = y >= 0.0 ? this.up * y : this.down * -y;
        final double shadeZ = this.z * Math.abs(z);
